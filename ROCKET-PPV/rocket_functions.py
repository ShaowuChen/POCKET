...
@njit("Tuple((float64[:],int32[:],float64[:],int32[:],int32[:], float64[:,:]))(int64,int64)")
...
def generate_kernels(input_length, num_kernels):
    ...
    # for comparing EA-SROCKET
    weights_norm1 = np.zeros((2,num_kernels))
    ...
    for i in range(num_kernels):
        ...
        # for comparing EA-SROCKET
        weights_norm1[0,i] = np.sum(np.abs(weights[a1:b1]))
        weights_norm1[1,i] = np.sqrt(np.sum(weights[a1:b1]**2))
    ...
    return weights, lengths, biases, dilations, paddings, weights_norm1
