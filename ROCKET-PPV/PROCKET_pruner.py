...
class PROCKETPruner(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X_training_transform, y_training):
        ...
        'different from ROCKET-PPV-MAX here'
        ...
        # P = np.linalg.inv(self.k * np.eye(n_sample) + X_training_transform @ X_training_transform.T)
        # P =  P @ X_training_transform
        # P = -1/self.k*X_training_transform.T @ P
        # P = np.eye(n_feature)/self.k + P
        ...
        self.thres = []
        self.W_norm = []
        self.thre_divide_largest = []
        for iter in range(self.epoch):
            
            # primal update: W
            W = P @ ( self.k*(Theta + U)  - X_training_transform.T @ (V - Y_demean))
            if iter!=0:
                self.norms2.append(np.linalg.norm(W-Theta))
            ...
            self.norms.append(np.linalg.norm(W-Theta))
            self.thres.append(threshold)
            self.W_norm.append(np.linalg.norm(W))
            self.thre_divide_largest.append(threshold/sorted_group_norm[0])
