...
class PROCKETPruner(BaseEstimator):
    ...
    def fit(self, X_training_transform, y_training):
        ...
        P = np.eye(n_feature)/self.k - 1/self.k*X_training_transform.T @ np.linalg.inv(
         ...
         # Woodbury formula /Sherman-Morrison-Woodbury formula
        # P = self.k * np.eye(n_sample) + X_training_transform @ X_training_transform.T
        # P = np.linalg.inv(P) 
        # P = np.eye(n_feature)/self.k - 1/self.k*X_training_transform.T @ P @ X_training_transform
